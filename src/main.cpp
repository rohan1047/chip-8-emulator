...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        for (int i = 0; i < 10; ++i) chip8.cycle();
        if (chip8.drawFlag) {
        ...
        if (chip8.sound_timer > 0) {
            if (!soundPlaying) { SDL_PauseAudioDevice(dev, 0); soundPlaying = true; }
        } else {
            if (soundPlaying) { SDL_PauseAudioDevice(dev, 1); soundPlaying = false; }
        }
